...
GglError publish_message(GglBuffer thing_name) {
    ...
    GglByteVec topic_vec = GGL_BYTE_VEC((uint8_t[TOPIC_BUFFER_LEN]) { 0 });
    GglError ret = ggl_byte_vec_append(&topic_vec, GGL_STR(TOPIC_PREFIX));
    ggl_byte_vec_append_cont(&ret, &topic_vec, thing_name);
    ggl_byte_vec_append_cont(&ret, &topic_vec, GGL_STR(TOPIC_SUFFIX));
    if (ret != GGL_ERR_OK) {
        return ret;
    }

    // build payload
    GglByteVec payload_vec = GGL_BYTE_VEC((uint8_t[PAYLOAD_BUFFER_LEN]) { 0 });
    ret = ggl_byte_vec_append(&topic_vec, GGL_STR(PAYLOAD_PREFIX));
    ggl_byte_vec_append_cont(&ret, &topic_vec, thing_name);
    ggl_byte_vec_append_cont(&ret, &topic_vec, GGL_STR(PAYLOAD_SUFFIX));
    if (ret != GGL_ERR_OK) {
        return ret;
    }
